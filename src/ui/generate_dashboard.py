...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .navbar .brand { display:flex; align-items:center; gap:8px; margin-right: auto; }
  .navbar .dots { display:flex; }
  .navbar .dots span { width:16px; height:16px; border-radius:50%; display:inline-block; margin-left:-6px; }
  .navbar .dots span:first-child { background: var(--mc-red); margin-left:0; }
  .navbar .dots span:last-child { background: var(--mc-orange); mix-blend-mode: multiply; }
...
<body>
  ...
  <section id="architecture" class="alt">
    ...
    <p class="body-text">The diagram below reflects the current implementation state. Replace
      <code>artifacts/architecture_diagram.png</code> with an updated export and refresh this page &mdash;
      no rebuild step is required for the image.</p>
    <div class="arch-wrap">
